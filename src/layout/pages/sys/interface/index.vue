<template>
    <admin-container>
        <template v-slot:header>
            <div>
                ...
                <el-form inline class="form-right">
                    <el-form-item>
                        <el-input placeholder="接口名称" v-model="search.name" clearable> </el-input>
                    </el-form-item>
                    <el-form-item>
                        <el-select v-model="search.disable" placeholder="可见性" clearable>
                            <el-option
                    ...
                    <el-form-item>
                        <el-select v-model="search.menu_id" placeholder="所属菜单" clearable>
                            <el-option v-for="item in menuOption" :key="item.id" :label="item.title" :value="item.id">
                            </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item>
                        <el-select v-model="search.role_id" placeholder="所属角色" clearable>
                            <el-option
        ...
        >
            <el-table-column type="selection" width="55" :selectable="isAuth"> </el-table-column>
            <el-table-column prop="name" label="名称" align="left" width="200"> </el-table-column>
            <el-table-column prop="path" label="路由" align="left"> </el-table-column>
            <el-table-column prop="method" label="请求方式" align="center" width="100"> </el-table-column>
            <el-table-column prop="disable" label="可见性" align="left" width="100">
            ...
            <el-table-column prop="menus" label="所属菜单" align="center" width="150">
                <template #default="scope">
                    <el-tag v-for="(i, k) in scope.row.menus" :key="i.menu_id">{{ i.name }}</el-tag>
                </template>
            </el-table-column>
            <el-table-column prop="roles" label="所属角色" align="center" width="150">
                <template #default="scope">
                    <el-tag v-for="(i, k) in scope.row.roles" :key="i.role_id">{{ i.name }}</el-tag>
                </template>
            </el-table-column>
            <el-table-column prop="description" label="描述" align="left" width="200"> </el-table-column>
            <el-table-column prop="mark" label="标识" align="left" width="200">
                <template #default="scope">
                    <el-tag>{{ scope.row.mark }}</el-tag>
                </template>
            </el-table-column>
            <el-table-column prop="content" label="操作" align="left" width="100">
                <template v-slot="scope">
                    <el-button circle title="编辑" :icon="Edit" @click="editInterface(auth.set, scope.row)"></el-button>
                    <el-button
